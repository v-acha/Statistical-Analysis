...
density_plot <- ggplot(filtered_data, aes(x = votehard, fill = party_affiliation)) +
  ...
  labs(title = "Density Plot of Voting Difficulty by Political Affiliation",
       x = "Voting Difficulty",
       y = "Density",
...
jitter_plot <- ggplot(filtered_data, aes(x = party_affiliation, y = votehard, color = party_affiliation)) +
  ...
  labs(title = "Jitter Plot of Voting Difficulty by Political Affiliation",
       x = "Party Affiliation",
       y = "Voting Difficulty",
...
To quantify the practical significance, we can calculate the effect size using the rank-biserial correlation, which is appropriate for the Wilcoxon rank-sum test:

The rank-biserial correlation provides a measure of effect size, indicating the strength and direction of the association between party affiliation and voting difficulty.
Given the small difference in means and the moderate overlap in distributions, the practical significance suggests a meaningful but not large effect.

```{r}
# Calculate effect size 
library(rcompanion) 
rank_biserial <- wilcoxonR (x = data$votehard[data$party_affiliation == "Democrat"], y = data$votehard[data$party_affiliation == "Republican"]) rank_biserial
```
